...
class FormulaParser:
    ...
    def classify_variables(param_df):
        """
        Classifies variables from the formulas into different categories based on their roles. 
        ...
        - 'end': Endogenous variables, variables determined within the system of the model.
        - 'exo': Exogenous variables, variables not determined within the system of the model.
        - 'reg': Any variables involved in regression equations (i.e., 'end' union 'exo').
        - 'lvo': Observed variables that are part of the structural model (intersection of 'reg' and 'obs').
        - 'lav': All variables treated as part of the structural model (union of 'lvo' and 'nob').
        - 'lox': Observed exogenous variables in the structural model (difference of 'lav' and union of 'nob', 'end', 'ind').
        - 'loy': Observed endogenous variables in the structural model (difference of 'lav' and union of 'nob', 'exo', 'ind').
        - 'lvx': True latent exogenous variables (difference of 'nob' and union of 'ind', 'end').
        - 'enx': Endogenous variables not considered exogenous (difference of 'end' and 'exo').

        These categories are not mutually exclusive and some variables may belong to multiple categories. 
        ...
        obs_var_names = all_var_names - nob_var_names              #observed variables i.e. ov  - rows of Lambda
        ...
        lvo_var_names = reg_var_names - nob_var_names #regression variables that are observed i.e. observed vars to add to structural model - the rows and cols of Lambda with identity
        lav_var_names = lvo_var_names | nob_var_names # all effectively latent variables i.e. treated as part of structural model - cols of lambda and row/col of beta and phi
        lox_var_names = lav_var_names - (nob_var_names | end_var_names | ind_var_names) #observed exog variables in structural part of model i.e.latent observed - fix Phi and beta at sample stats
        loy_var_names = lav_var_names - (nob_var_names | exo_var_names | ind_var_names) #observed endog variables in structural part of model i.e.latent observed
        lvx_var_names = nob_var_names - (ind_var_names | end_var_names) #true latent exog vars 
        enx_var_names = end_var_names - exo_var_names #endo but not exo var names
