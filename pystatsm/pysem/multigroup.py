...
from .derivatives import _dloglike_mu, _d2loglike_mu0, _d2loglike_mu1, _d2loglike_mu2, _dsigma_mu, _d2sigma_mu #from .cov_derivatives import _d2sigma, _dsigma, _dloglike, _d2loglike
...
class SEM:
    ...
    def make_derivative_matrices(self):
        ...
        self.n_group_theta = self.mspec.free_to_theta.shape[0]
        self.dSm = np.zeros((self.mspec.n_groups, self.p2+self.p, self.nf))
        self.d2Sm = np.zeros((self.mspec.n_groups, self.p2+self.p, self.nf, self.nf))
    ...
    def _theta_to_free(self, theta):
        free = theta[self.mspec._unique_locs]
        return free
