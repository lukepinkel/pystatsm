...
class ModelData(object):
    ...
    def subset_and_order(self, variables):
        ...
        if hasattr(self, "data_df"):
            self.data_df = self.data_df.loc[:, variables]
    ...
    def from_samplestats(cls, sample_cov=None, sample_mean=None, n_obs=None, ddof=0):
        n_groups = 1
        if isinstance(sample_cov, dict) and isinstance(sample_mean, dict) and (
                n_obs is None or isinstance(n_obs, dict)):
            n_groups = len(sample_cov)
        elif isinstance(sample_cov, (pd.DataFrame, np.ndarray)) and isinstance(sample_mean, (
        pd.DataFrame, np.ndarray, pd.Series)) and (n_obs is None or isinstance(n_obs, int)):
