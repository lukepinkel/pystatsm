...
class SimulatedSEM:
    def __init__(self, formula, n_groups, n_obs_min=1000, n_obs_max=5000, rng=None):
        ...
        self.n_obs = self.simulate_group_sizes()
        self.set_model_spec()
    ...
    def generate_data_from_modelspec(self):
        ...
        dfs = []
        for i in range(self.n_groups):
            arr = rng.multivariate_normal(mean= mu[i].values.flatten(), cov=Sigma[i].values, size=self.n_obs[i])
    ...
    def simulate(self):
        ...
        self.update_model_spec(self.param_df)
        df = self.generate_data_from_modelspec()
        return df
