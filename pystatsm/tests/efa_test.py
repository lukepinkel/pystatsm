...
def test_efa():
    ...
    model = FactorAnalysis(X, n_factors=n_facs, rotation_method="varimax", 
                           rotation_type="oblique")
    model.fit()
    
    Lambda = np.array([[ 0.95740655,  0.0452998 , -0.00719972],
                       [ 0.97006582,  0.0429361 , -0.00497628],
                       [ 0.95513318,  0.04500033, -0.00471208],
                       [ 0.94398874,  0.03836091,  0.00305088],
                       [ 0.92086791,  0.04346582, -0.00387235],
                       [ 0.01915415,  0.91162981,  0.04844959],
                       [ 0.02373153,  0.88068969,  0.04740113],
                       [ 0.03017109,  0.9021467 ,  0.05053064],
                       [ 0.03202448,  0.81306424,  0.04350056],
                       [ 0.03189557,  0.74775305,  0.04308543],
                       [-0.00555388,  0.01900303,  0.81341605],
                       [ 0.00387156,  0.02814028,  0.80791127],
                       [-0.00639639,  0.00573269,  0.66831427],
                       [-0.0023957 ,  0.01852341,  0.71999204],
                       [-0.00222832,  0.02100683,  0.54274256]])
    Phi = np.array([[ 1.        ,  0.22296494, -0.0120192 ],
                    [ 0.22296494,  1.        ,  0.24789515],
                    [-0.0120192 ,  0.24789515,  1.        ]])
    assert(np.allclose(model.L, Lambda, atol=1e-5))
    assert(np.allclose(model.Phi, Phi, atol=1e-5))
