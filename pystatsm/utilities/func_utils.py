...
# 32-point Gauss-Legendre nodes/weights for binorm_l2_jit, hoisted to module
# scope so numba treats them as compile-time constants and the (32, 3) array
# isn't allocated on every call.
_GL32_W = np.array([
    0.0494723666239310, 0.0494118330399182, 0.0492303804237476,
    0.0489284528205120, 0.0485067890978838, 0.0479664211379951,
    0.0473086713122689, 0.0465351492453837, 0.0456477478762926,
    0.0446486388259414, 0.0435402670830276, 0.0423253450208158,
    0.0410068457596664, 0.0395879958915441, 0.0380722675843496,
    0.0364633700854573, 0.0347652406453559, 0.0329820348837793,
    0.0311181166222198, 0.0291780472082805, 0.0271665743590979,
    0.0250886205533450, 0.0229492710048899, 0.0207537612580391,
    0.0185074644601613, 0.0162158784103383, 0.0138846126161156,
    0.0115193760768800, 0.0091259686763267, 0.0067102917659601,
    0.0042785083468638, 0.0018398745955771,
])
_GL32_X = np.array([
    0.0000000000000000, 0.0494521871161596, 0.0987833564469453,
    0.1478727863578720, 0.1966003467915067, 0.2448467932459534,
    0.2924940585862514, 0.3394255419745844, 0.3855263942122479,
    0.4306837987951116, 0.4747872479948044, 0.5177288132900333,
    0.5594034094862850, 0.5997090518776252, 0.6385471058213654,
    0.6758225281149861, 0.7114440995848458, 0.7453246483178474,
    0.7773812629903724, 0.8075354957734567, 0.8357135543195029,
    0.8618464823641238, 0.8858703285078534, 0.9077263027785316,
    0.9273609206218432, 0.9447261340410098, 0.9597794497589419,
    0.9724840346975701, 0.9828088105937273, 0.9907285468921895,
    0.9962240127779701, 0.9992829840291237,
])
...
def sizes_to_ind_arrs(sizes, keys=None):
    """
    Return a list of slices given an array of sizes.
    ...
    """
    inds = sizes_to_inds(sizes)
    keys = range(len(inds)-1) if keys is None else keys
    ind_dict = {key:np.arange(inds[i], inds[i+1]) for i, key in enumerate(keys)}
    return ind_dict
...
def binorm_l2_jit(h, k, r):
    """
     ...
     """
    r2 = r / 2.0
    gsum = _GL32_W[0] * binorm_dl_jit(h, k, (1.0 + _GL32_X[0]) * r2)
    for i in range(1, 32):
        w = _GL32_W[i]
        root = _GL32_X[i]
        gsum += w * binorm_dl_jit(h, k, (1.0 - root) * r2)
        gsum += w * binorm_dl_jit(h, k, (1.0 + root) * r2)
    likelihood = r2 * gsum
...
def binorm_cdf_region(lower, upper, r):
    ...
    """
    # Stack the four corners into one array and invoke the JIT'd CDF once
    # instead of four times — saves three full numba dispatches per call.
    if np.ndim(r) > 0:
        r = r[0]
    h = np.stack([np.atleast_1d(upper[0]), np.atleast_1d(upper[0]),
                  np.atleast_1d(lower[0]), np.atleast_1d(lower[0])])
    k = np.stack([np.atleast_1d(upper[1]), np.atleast_1d(lower[1]),
                  np.atleast_1d(upper[1]), np.atleast_1d(lower[1])])
    p = _binorm_cdf_arr(h, k, r)
    pr = p[0] - p[1] - p[2] + p[3]
    if np.ndim(upper[0]) == 0:
        pr = pr[0]
    return pr
